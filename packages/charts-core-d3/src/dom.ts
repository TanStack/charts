...
export function mountChart<TDatum, TInput = undefined>(
  ...
  const runtime = createChartRuntime<TDatum, TInput>()
  ...
  const configureObserver = () => {
    ...
    observer = new ResizeObserverConstructor(() => {
      ...
      if (width <= 0 || width === scene.width || resizeFrame !== undefined)
        return
      ...
      resizeFrame = view.requestAnimationFrame(() => {
        ...
        if (nextWidth > 0 && nextWidth !== scene.width) render()
      })
    })
  ...
  const handlePointerPosition = (clientX: number, clientY: number) => {
    ...
    updateFocus(resolvePointerFocus(x, y, maxDistance))
  }
  const handlePointerMove = (event: PointerEvent) =>
    handlePointerPosition(event.clientX, event.clientY)
  const handleMouseLeave = () => updateFocus([])
  ...
  return {
    ...
    destroy() {
      ...
      if (resizeFrame !== undefined) {
        container.ownerDocument.defaultView?.cancelAnimationFrame?.(resizeFrame)
      }
      cancelAnimation()
      runtime.destroy()
  ...
  function createScene(): ChartScene<TDatum> {
    ...
    return runtime.render(options.definition, options.input as TInput, {
      width,
      height:
        options.height ??
