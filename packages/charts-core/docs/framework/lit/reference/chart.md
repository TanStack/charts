...
Static definitions reject `input`; dynamic definitions require the exact
`input` type declared by `defineChart<Input>()`.
...
`ChartCommonProps` contains common host and presentation props.
