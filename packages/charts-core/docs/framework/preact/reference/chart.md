...
`StaticChartProps` adds a static definition and rejects `input`.
`DynamicChartProps` adds a dynamic definition and requires `input`.
`ChartProps` is their union. `ChartPresentationProps` contains `className` and
