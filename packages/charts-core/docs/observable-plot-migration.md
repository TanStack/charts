...
ground-up D3-native engine, not a Plot fork and not a complete compatibility
layer.
...
Then use this package’s recipes for exact TanStack syntax.
...
| linear, log, symlog, sqrt, time, UTC, band, point | raw D3 scales supplied on each materialized axis     |
| ordinal and continuous color                      | raw D3 color scales; ordinal theme default available |
| `bin`, `group`, `stack`                           | direct D3 output adapted to channels in `prepare`    |
| facets                                            | `facetChart` or the composable `facet` mark          |
| `pointer` tip                                     | omit `focus` for one nearest 2D point                |
| `pointerX` or `pointerY` tip                      | `focusNearestX` or `focusNearestY`                   |
| grouped cross-section tip                         | `focusX` or `focusY`                                 |
| activation                                        | host focus and selection callbacks                   |
...
## Not in the supported grammar yet
...
depends heavily on those capabilities. Migrate charts composed from the
supported subset one layer at a time, preserving original data and stable
keys.
