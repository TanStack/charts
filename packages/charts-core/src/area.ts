...
>(
  source: Iterable<TDatum>,
  options:
    AreaYCallOptions<TDatum, TXChannel, TXScaleId, TYScaleId> | undefined,
): CartesianChartMark<
  TDatum,
...
): CartesianChartMark<TDatum, any, any, any, any, AreaYOptions<TDatum>> {
        ...
        render: ({ scales, color: resolveColor }) => {
          ...
          for (const [groupKey, indices] of groups) {
            ...
            for (const datumIndex of indices) {
              ...
              const y = scales[yScale]!.map(y2Value)
              top.push([x, y])
              bottom.push([x, scales[yScale]!.map(y1Value)])
